    ...
    <div className="container mx-auto px-3 sm:px-6 lg:px-8 py-4 sm:py-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
        {filteredCustomers.map((customer) => {
          ...
          const formatCpfCnpjDisplay = (value: string | undefined) => {
            if (!value) return "";
            const digits = value.replace(/\D/g, "");
            if (digits.length <= 11) {
              return digits
              ...
              <CardHeader>
                <div className="flex flex-col gap-1">
                  ...
                  {customer.nome_fantasia && (
                    <p className="text-sm text-muted-foreground">{customer.nome_fantasia}</p>
                  )}
                </div>
                {customer.cpf_cnpj && (
                  <p className="text-sm font-mono text-muted-foreground">{formatCpfCnpjDisplay(customer.cpf_cnpj)}</p>
                )}
              </CardHeader>
              <CardContent className="space-y-3">
                ...
                {customerContacts.length > 0 && (
                  <div className="space-y-1">
                    <span className="text-xs font-medium text-muted-foreground uppercase">Contatos</span>
                    <div className="space-y-1">
                      {customerContacts.map((contact) => (
                        <div key={contact.id} className="text-sm flex items-center gap-2">
                          ...
                          {contact.responsavel && <span className="font-medium">{contact.responsavel}</span>}
                          {contact.telefone && <span className="text-muted-foreground">{contact.telefone}</span>}
                        </div>
                      ))}
                    </div>
                  </div>
